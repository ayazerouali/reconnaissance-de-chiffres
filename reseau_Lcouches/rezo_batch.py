...
def model_save_py(X, Y, batch_size, layers_dim, learning_rate, iterations, X_test, Y_test):
    ...
    parameters = L.L_layer_model(X,Y,batch_size,layers_dim,learning_rate,iterations)
    accu = accuracy(X_test, Y_test, parameters)
    
    file = open("model_batch.py","w")

    filepath = os.path.join("/Users/thomasb/Desktop/CS_2A/projet_2A/apprentissage", "model_batch.py")
...
#model_save_py(X, Y, 100, layers_dim, 0.03, 25, images, labels)
...
def model_save_h5(X,Y,batch_size, layers_dim,learning_rate,iterations):
    parameters = L.L_layer_model_batch(X,Y,batch_size, layers_dim,learning_rate,iterations)
